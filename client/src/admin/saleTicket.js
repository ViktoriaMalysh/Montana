...
function Tickets() {
  ...
  useEffect(() => {
    if (discount !== 0) console.log("price:", price, "sale:", discount);
  }, [discount]);
  ...
  const createDiscount = (name1, address1, locality1, price1) => {
    console.log("CreateTicket", name1, address1, locality1, price1);
    setName(name1);
    setAddress(address1);
    setLoc(locality1);
    setPrice(price1.slice(1));
    if (value === "percentage") {
      const percentage = 100 - amount;
      const discount = Math.round((price1.slice(1) / 100) * percentage);
      setDiscount(discount);
    } else if (value === "fixed") {
      const discount = price1.slice(1) - amount;
      setDiscount(discount);
    }
  };
...
<br></br>
                ...
                <Card.Text>
                  Price wich discount:{" "}
                  <span style={{ fontStyle: "italic" }}>{'$'}{discount}</span>
                </Card.Text>

                <Card.Text>
                  Amount of discount:{" "}
                  {value === "percentage" ? (
                    <span style={{ fontStyle: "italic" }}>{amount}{"%"}</span>
                  ) : (
                    <span style={{ fontStyle: "italic" }}>{"$"}{amount}</span>
                  )}
                </Card.Text>
              ...
              <p className="textmmessage">
                If all the data is correct, then create discount:
              </p>
              <Button
                ...
                onClick={() =>
                  dispatch(
                    addTicket(store.users.userId, name, address, loc, price)
                  )
                }
              >
                Create discount
                  ...
                  <div className="buttform">
                    <Row className="align-items-center">
                      <Col xs="auto" className="my-1">
                        <Form.Label>
                          <h4 className="selectDis">
                            Select discount criterion:
                          </h4>
                        </Form.Label>
                      </Col>
                      <Col className="my-1">
                    ...
                    {value ? (
                      ...
                      >
                        <Form.Label>
                          <h4>Enter amount of discount:</h4>
                        </Form.Label>
